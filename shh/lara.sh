...
APP_REPO=git@github.com:alphaolomi/hostel-sys.git
APP_NAME=HOSTEL
APP_NAME_SHORT=hostel33
APP_ENV=production
APP_DEBUG=false
APP_URL=http://167.172.126.45
...
sudo apt install php-cli php-mbstring php-xml php-bcmath php-mysql -y > install-php.log
echo "- installing composer unzip curl git..."
sudo apt install composer unzip curl git -y > install-misc.log
echo ""
...
echo "- add disable StrictHostKeyChecking"
cat > ~/.ssh/config <<EOF
Host github.com
